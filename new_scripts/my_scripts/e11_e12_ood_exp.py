#!/usr/bin/env python3
"""
E11 (OOD Generalization: GPQA, MMLU) + E12 (Non-Math Reasoning: BBH, HotpotQA, MuSiQue)
Only Qwen2.5-3B-Instruct.  Reuses the GPU-pool runner from 11_steer_all_datasets_exp.py.
"""
...
E12_TASKS = ["bbh_cot_zeroshot", "longbench_hotpotqa", "longbench_musique"]
...
def run_one(gpu_id: int, job: dict, output_root: Path, batch_size: int,
            state: TaskStateManager, limit: Optional[int]):
...
def main():
    parser = argparse.ArgumentParser(description="E11+E12 OOD experiments for Qwen2.5-3B-Instruct")
    parser.add_argument("--experiments", type=str, nargs="+",
                        choices=["E11", "E12", "ALL"], default=["ALL"])
    parser.add_argument("--modes", type=str, nargs="+",
                        choices=["BASELINE", "GPT_REWRITE", "LARGE_MODEL", "ALL"],
                        default=["ALL"])
    ...
    parser.add_argument("--output_root", type=str, default="exps/steer_runs_e11_e12")
    parser.add_argument("--status_file", type=str, default="exps/steer_runs_e11_e12/experiment_status.json")
    ...
    modes = args.modes
    if "ALL" in modes:
        modes = ["BASELINE", "GPT_REWRITE", "LARGE_MODEL"]
