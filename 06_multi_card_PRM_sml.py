...
# CONFIGS
# ============================================================
PREFIX = "/common/users/sl2148/Public/yang_ouyang/projects/lm-evaluation-harness/lm_eval/models/eval_grid_qwen_family_lib_large_lambda/"
FOLDER = PREFIX + "prm"
...
BASE_DIR = PREFIX + "gsm8k_cot_zeroshot"
# lam_values = [
#     "lam-0p2","lam-0p4","lam-0p6", #,"lam-0p4", "lam-0p5",
#     "BASELINE",
#     "lam0p2","lam0p4","lam0p6"#,"lam0p4", "lam0p5"
# ]

# STEP = 0.5
# LAM_IDXS = list(range(-20, 21))          # 整数 key
# STEER_LAMBDAS = [i * STEP for i in LAM_IDXS]
STEP = 1
...
def lam_to_str(lam: float) -> str:
    ...
    # 1. 先保留两位小数
    s = f"{abs(lam):.2f}"          # 1.0 -> "1.00", 0.5 -> "0.50"
    
    # 2. 去除末尾的 '0'
    s = s.rstrip("0")              # "1.00" -> "1.", "0.50" -> "0.5"
    
    # 3. 关键逻辑：如果结果以 '.' 结尾（说明是整数），补回一个 '0'
    if s.endswith("."):
        s += "0"                   # "1." -> "1.0"
    
    # 4. 最后替换小数点
    s = s.replace(".", "p")        # "1.0" -> "1p0", "0.5" -> "0p5"
...
MODEL_TO_LAYERS = {
    # "Qwen2.5-0.5B-Instruct": [12, 20, 24],
    # "Qwen2.5-1.5B-Instruct": [14, 24, 28],
    "Qwen2.5-3B-Instruct":   [17, 31, 35],
...
# all_layers = [1]
# MODEL_TO_LAYERS = {
#     "Qwen2.5-0.5B-Instruct": all_layers,
#     "Qwen2.5-1.5B-Instruct": all_layers,
#     "Qwen2.5-3B-Instruct":   all_layers,
#     "Qwen2.5-7B-Instruct":   all_layers,
#     "Qwen2.5-14B-Instruct":  all_layers,
# }
...
SAVE_DIR = "/common/users/sl2148/Public/yang_ouyang/projects/lm-evaluation-harness/lm_eval/models/eval_grid_qwen_family_lib/prm"
MODEL_FILES = [
